    ...
    */
        ...
        try
        {
	        if (keyManagers != null && keyManagers.length > 0)
	        {
	        	...
	        	if (certChain != null && certChain.length > 0)
	        	{
	        		...
	        		{
		            	byte[] cert = certChain[i].getEncoded();
		            	TlsUtils.writeUint24(cert.length, bout);
		            	bout.write(cert);
	        		}
	        		byte data[] = bout.toByteArray();
	        		TlsUtils.writeUint24(data.length+3, bos);
	        		TlsUtils.writeUint24(data.length, bos);
	        		bos.write(data);
	        	}
	        }
	        else
    ...
    private void sendClientKeyExchange(byte[] keData) throws IOException
    {
		...
		if (data != null)
		{
	        ...
	        TlsUtils.writeUint16(data.length, bos);
	        bos.write(data);
     ...
     */
    public void connect(ByteArrayOutputStream dest, CertificateVerifyer verifyer) throws IOException
    {
